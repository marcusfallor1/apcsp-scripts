...

{
  ...
  for {
  gradesfile = fopen("gradesfile.out", "w");
  if (gradesfile == NULL) {
    printf("error - failed to open file for writing\n");
    return 1;
  }
  
  fprintf(gradesfile, "Grade results output\n");
  ...
  if ( num == 2 )
	...
	return((dg-(bw*b*.01))/aw)*100;
      ...
      fprintf(gradesfile, "To get a %f percent in the class you need end with a %f percent overall in this category\n", dg, a);
      float NesPointCat2()
      {
	return(.01*a*tp + .01*a*fk - pg);
      ...
      fprintf(gradesfile, "You need to get %f points out of %f points on your last assignment to get a %f in the class\n", x, fk, dg);
      float NesGradeCat2()
      {
	return(100*(x/fk));
      }
      ...
      fprintf(gradesfile, "You need a %f percent on your last assignment to get a %f percent in the class\n",g, dg);
    }
  if ( num == 3 )
    {
      float NesPercCat3()
